...
public class PlayerCombat : MonoBehaviour
{
    ...
    [Header("Authored spell assets")]
    [SerializeField] private SpellDefinition[] spells;
    private SpellDefinition Spell(SpellType type) { if (spells != null) foreach (var spell in spells) if (spell != null && spell.spellType == type) return spell; return null; }
    ...
    private void CastProjectile(SpellType spellType, float chargePercent)
    {
        ...
        var authored = Spell(spellType);
        if (authored != null && authored.projectilePrefab != null)
        {
            var shot = Instantiate(authored.projectilePrefab, spawnPosition, Quaternion.identity);
            var scale = shot.transform.localScale; scale.x *= direction; shot.transform.localScale = scale;
            shot.GetComponent<SpellProjectile>().Launch(direction, projectileSpeed, GetProjectileRange(spellType, chargePercent), GetDamage(spellType, chargePercent));
            GameAudio.PlaySfx(spellType == SpellType.Triangle ? "triangleSF" : "squareSF", transform.position, .85f);
            return;
        }
    ...
    private void CastCircleShield()
    {
        ...
        var authored = Spell(SpellType.Circle);
        if (authored != null && authored.effectPrefab != null)
        {
            var authoredShield = Instantiate(authored.effectPrefab, transform);
            Physics2D.IgnoreCollision(authoredShield.GetComponent<Collider2D>(), GetComponent<Collider2D>(), true);
            authoredShield.GetComponent<CircleShield>().Activate(circleShieldHealth, circleShieldDuration);
            GameAudio.PlaySfx("shieldOnSF", transform.position, .85f); return;
        }
    ...
    private void CastKnifeAttack()
    {
        ...
        var authored = Spell(SpellType.Knife);
        if (authored != null && authored.effectPrefab != null)
        {
            var knife = Instantiate(authored.effectPrefab, spawnPosition, Quaternion.identity);
            var scale = knife.transform.localScale; scale.x *= direction; knife.transform.localScale = scale;
            knife.GetComponent<MeleeAttack2D>().Launch(gameObject, knifeDamage, knifeResourceValue, .15f);
            GameAudio.PlaySfx("knifeMissSFX", transform.position, .8f); return;
        }
