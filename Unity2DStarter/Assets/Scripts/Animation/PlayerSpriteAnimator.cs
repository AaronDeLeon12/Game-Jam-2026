...
{
    [SerializeField] private Animator authoredAnimator;
    private string authoredState;
    private float authoredLockUntil;
    private bool authoredDead;
    ...
    public bool UsesAuthoredAnimator => authoredAnimator != null;
    public bool HasLoadedSprites => authoredAnimator != null || walkFrames.Count > 0;
    ...
    private void Awake()
    {
        ...
        if (authoredAnimator != null) return;
        LoadWalkFrames();
        LoadStateFrames();
        ApplyIdleFrame();
    ...
    private void Update()
    {
        ...
        {
            authoredAnimator.speed = PauseMenu.IsPaused || (GameModal.IsOpen && !authoredDead) ? 0 : 1;
            if (PauseMenu.IsPaused || authoredDead) return;
            spriteRenderer.flipX = movement != null && movement.FacingDirection < 0;
            if (Time.unscaledTime < authoredLockUntil) return;
            string next = movement != null && !movement.IsGroundedNow ? "Jump" : movement != null && movement.IsDucking ? (Mathf.Abs(body.linearVelocity.x) > minimumWalkSpeed ? "CrouchWalk" : "Crouch") : Mathf.Abs(body.linearVelocity.x) > minimumWalkSpeed ? "Walk" : "Idle";
            PlayAuthored(next);
            return;
        }
    ...
    {
        if (authoredAnimator != null) { PlayAuthored("Vanish", duration); yield return new WaitForSecondsRealtime(duration); yield break; }
        yield return PlayExternalAnimation(teleportVanishFrames, duration);
    }

    public IEnumerator PlayTeleportAppear(float duration)
    {
        if (authoredAnimator != null) { PlayAuthored("Appear", duration); yield return new WaitForSecondsRealtime(duration); yield break; }
        yield return PlayExternalAnimation(teleportAppearFrames, duration);
    }

    public void PlaySpellAttack(float duration = 0.28f)
    {
        if (authoredAnimator != null) { PlayAuthored("Spell", duration); return; }
    ...
    {
        if (authoredAnimator != null) { PlayAuthored("Death", duration); authoredDead = true; return; }
    ...
    public void ResetVisualState()
    {
        ...
        authoredLockUntil = 0;
        authoredState = null;
        if (authoredAnimator != null) { authoredAnimator.speed = 1; PlayAuthored("Idle"); }
        else ForceRefresh();
    }

    private void PlayAuthored(string state, float duration = 0)
    {
        if (authoredState != state || duration > 0) { authoredAnimator.Play(state, 0, 0); authoredState = state; }
        if (duration > 0) authoredLockUntil = Time.unscaledTime + duration;
    }
