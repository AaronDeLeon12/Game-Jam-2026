...
public class FinalBoss : MonoBehaviour, IDamageable
{
    ...
    private void LaunchOne(Vector2 target, float flightTime, float gravity)
    {
        ...
        {
            var shot = Instantiate(definition.projectilePrefab, transform.position + new Vector3(facingDirection * .5f, projectileSpawnYOffset, 0), Quaternion.identity);
            shot.GetComponent<SpriteRenderer>().flipX = facingDirection < 0;
            shot.GetComponent<BossProjectile>().Launch(target, projectileDamage, flightTime, gravity); return;
        }
