...
public class KitingShooterEnemyAI : MonoBehaviour
{
    ...
    private void Awake()
    {
        if (definition != null)
        {
            ...
            preferredDistance = definition.attackRange;
            fireInterval = definition.attackCooldown;
            projectileDamage = definition.attackDamage;
        }
        if (GetComponent<Animator>() != null) spriteRenderer = GetComponent<SpriteRenderer>();
        else SetupVisual();
    ...
    private void SetupVisual()
    {
        ...
        animator.ConfigureFrameFiles(
            "Enemies",
            new[] { "unicorn1NoBg", "unicorn2NoBg", "unicorn3NoBg", "unicorn4NoBg", "unicorn5NoBg", "unicorn6NoBg" },
            8f,
            true,
            true,
            256f,
    ...
    private void FireAtPlayer()
    {
        ...
        {
            var shot = Instantiate(definition.projectilePrefab, transform.position + (Vector3)(direction * .75f), Quaternion.identity);
            shot.GetComponent<SpriteRenderer>().flipX = direction.x > 0;
            shot.GetComponent<EnemyProjectile>().Launch(direction, projectileSpeed, projectileRange, projectileDamage); return;
        }
