...
public class SaveVendor : MonoBehaviour, IInteractable
{
    ...
    {
        if (AuthoredMenuCanvas.Instance != null)
        {
            SessionStats.Record("save_vendor_interactions");
            GameModal.Open();
            AuthoredMenuCanvas.Instance.ShowDialogue(greetings[Random.Range(0, greetings.Length)], () => AuthoredMenuCanvas.Instance.ShowSave());
            return;
        }
        SessionStats.Record("save_vendor_interactions");
