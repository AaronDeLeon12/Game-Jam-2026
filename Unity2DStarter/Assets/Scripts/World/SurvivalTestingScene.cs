...
public class SurvivalTestingScene : MonoBehaviour
{
    ...
    {
        if (kind == EnemyKind.Mantis)
        {
            GameObject mantis = MantisEnemyFactory.Create(position, parent, "Survival Mantis " + index);
            SurvivalEnemyActivator mantisActivator = mantis.AddComponent<SurvivalEnemyActivator>();
            mantisActivator.Configure(enemyActivationRange);
            return;
        }
