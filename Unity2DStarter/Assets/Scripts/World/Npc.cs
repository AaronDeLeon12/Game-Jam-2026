...
public class Npc : MonoBehaviour, IInteractable
{
    ...
    {
        talking = false;
        AuthoredMenuCanvas.Instance?.HideDialogue();
        GameModal.Close();
        dialogueIndex = 0;
    ...
    {
        if (AuthoredMenuCanvas.Instance == null || !talking) return;
        var lines = CurrentDialogues();
        if (dialogueIndex < lines.Length) AuthoredMenuCanvas.Instance.ShowNpcDialogue(lines[dialogueIndex].text, AdvanceDialogue);
    }
    private void OnGUI()
    {
        if (AuthoredMenuCanvas.Instance != null) return;
        if (!talking)
        {
            return;
        }
